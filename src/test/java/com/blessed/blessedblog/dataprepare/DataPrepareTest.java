package com.blessed.blessedblog.dataprepare;

import com.blessed.blessedblog.entity.*;
import com.blessed.blessedblog.repository.AddressRepository;
import com.blessed.blessedblog.repository.CityRepository;
import com.blessed.blessedblog.repository.CountryRepository;
import com.blessed.blessedblog.repository.PersonalInformationRepository;
import com.blessed.blessedblog.repository.ProvinceRepository;
import com.blessed.blessedblog.repository.RoleRepository;
...
public class DataPrepareTest {
    ...
    @Test void testInsertRoleAdmin() {
        Optional<Role> roleAdmin = roleRepository.findByRoleName("ROLE_ADMIN");
    ...
    @Test void testInsertProvinces() {
        ...
        if (allProvinces.isEmpty()) {
            ...
            for (String province : split) {
                ...
                temp.setProvinceName(province);
                Province savedProvince = provinceRepository.saveAndFlush(temp);
                provinceSet.add(savedProvince);
            }
            Country china = countryRepository.findById(1).get();
            china.setProvinces(provinceSet);
            countryRepository.saveAndFlush(china);
        }
    }
    @Test void testInsertCities() {
        ...
        if (allCities.isEmpty()) {
            ...
            for (int i = 1; i < 57; i += 2) {
                Province province = provinceRepository.findByProvinceNameLike(split[i]).get();
    ...
    @Test void testInsertAdmin() {
        ...
        personalInformation.setNickName("Blessed");
        personalInformation.setEducation("NUIST");
        personalInformation.setEmail("blessedwmm@gmail.com");
        personalInformation.setName("王美民");
        personalInformation.setPhoneNumber("15850720606");
        personalInformation.setPostcode(224161);

        Address address = new Address();
        address.setCountry("中国");
        address.setProvince("江苏省");
        address.setCity("南京市");
        address.setAddress("江苏省南京市浦口区宁六路219号");
    ...
    @Test void testBatchInsertUsers() {
        ...
        for (User user : users) {
            user = new User();
            String username = UuidUtils.getUuidString().substring(0, 20);
            String name = username.substring(0, 10);
            String nickName = username.substring(0, 16);
            ...
            personalInformation.setAddress(savedAddress);
            personalInformation.setEducation("NUIST");
            personalInformation.setPostcode(224101);
            personalInformation.setPhoneNumber("15850720607");
            PersonalInformation savedPersonalInformation = personalInformationRepository.saveAndFlush(personalInformation);
            user.setRoles(roles);
            user.setUsername(username.substring(10));
